...
class GeneticAlgorithm:
    def __init__(self, elitism = 20, mutation = 0.01, fitness_function_degree = 1.0):
        ...
        """
        Genetic Algorithm Optimizer. Finds the path that minimizes distance traveled between nodes.
    ...
    def __initialize(self, nodes: list, popsize: int, gens: int, mode: str):
        """
        Creates the initial population of individuals. Each individual consists of nodes and has its length.
        """
        for node in nodes:
    ...
    def __evaluate_selection_probability(self):
        ...
        :return: List of probabilities of choosing individuals from the population carried to the selection stage.
        """
    ...
    def __proportionate_selection(self):
        ...
        :return: List of individuals that have passed the selection stage.
        """
    ...
    def __crossover(self, first_parent, second_parent):
        """
        Ordered crossover. Randomly selects a subset of the first parent genes and then fill the remainder with the genes from the second parent in the order in which they appear, 
    ...
    def __crossover_population(self, mating_pool: list):
        """
        Function carrying crossover over the entire selected population.
    ...
    def __mutate(self, individual):
        """
        Swap mutation. With a specified low probability (mutation rate), two nodes will swap places in our individual. Swap does not include the first and last node.
    ...
    def __mutate_population(self, offsprings: list):
        """
    ...
    def __evaluate(self, next_generation: list):
        """
        In each generation, the best individual needs to be found. This function does exactly that.
    ...
    def fit(self, nodes: list, popsize = 50, gens = 500, mode = 'proportionate', verbose = True, decimal = 2):
        """
        The core function of the optimizer. It fits GAO to a specific list of nodes. 
        ...
        for gen in range(gens+1):
            ...
            if verbose: print(f"Generation {gen}/{gens} | Score: {round(best_score, decimal)} | Best so far: {round(self.best_score, decimal)} | {round(time.time() - start_gen, decimal)} s")
    ...
    def show_graph(self, fitted = True, figsize = (10,5), dpi = 200):
        """
        Shows the graph of nodes that GAO is working on.
